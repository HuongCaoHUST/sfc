...
gst_yoloclassification_class_init (GstYoloClassificationClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_MOTION_THRESHOLD,
      g_param_spec_double ("motion-threshold", "Motion Threshold", "Threshold for motion detection",
          0.0, G_MAXDOUBLE, 1000.0, (GParamFlags)(G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS)));
...
gst_yoloclassification_init (GstYoloClassification * filter)
{
  ...
  filter->motion_threshold = 1000.0;
}
...
static void gst_yoloclassification_finalize (GObject * object)
{
    ...
    if(filter->prev_frame) {
        delete static_cast<cv::Mat*>(filter->prev_frame);
        filter->prev_frame = NULL;
    }
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_MOTION_THRESHOLD:
      filter->motion_threshold = g_value_get_double(value);
      GST_INFO_OBJECT(filter, "Motion threshold set to: %f", filter->motion_threshold);
      break;
    default:
    ...
    GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
    ...
    case PROP_CONF_THRESHOLD:
      ...
      break;
    case PROP_MOTION_THRESHOLD:
      g_value_set_double(value, filter->motion_threshold);
      break;
    default:
      G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
      break;
...
gst_yoloclassification_transform_ip (GstBaseTransform * base, GstBuffer * outbuf)
{
  ...
  if (gst_buffer_map (outbuf, &map, (GstMapFlags)GST_MAP_READWRITE)) {
    ...
    if (filter->prev_frame == NULL) {
        filter->prev_frame = new cv::Mat(gray_frame.clone());
    } else {
        cv::Mat* prev_gray_frame = static_cast<cv::Mat*>(filter->prev_frame);
        cv::Mat frame_delta;
        cv::absdiff(*prev_gray_frame, gray_frame, frame_delta);
        double motion_score = cv::sum(frame_delta)[0];

        if (motion_score > filter->motion_threshold) {
            classifications = filter->classifier->classify(frame, filter->conf_threshold);
        }
